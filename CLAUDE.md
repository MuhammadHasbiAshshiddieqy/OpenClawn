...
- **Stack:** Python 3.12, FastAPI, HTMX, SQLite (aiosqlite), Pydantic, httpx, tenacity, structlog. Final.
  ...
  - **Pengecualian sadar #4 (disetujui owner, 2026-07-28):** `cryptography` (Fernet) untuk enkripsi-at-rest `security/vault.py::encrypt_secret`/`decrypt_secret`, dipakai `mcp_servers.env` (env var subprocess MCP per-baris, tak bisa lewat Vault env-var-backed biasa karena diisi user lewat API/UI, bukan dikenal saat deploy). Menutup gap `CLAUDE.md §1.2` "credential tidak pernah masuk tabel DB" yang ditemukan audit produksi 2026-07-27. Dipilih atas implementasi AES/kripto manual sendiri (risiko bug keamanan jauh lebih tinggi ketimbang library teraudit). Key dari env var `OPENCLAWN_ENCRYPTION_KEY` (wajib diisi bila ada server MCP dengan env non-kosong; kosong + env kosong tetap jalan tanpa error). Baris lama pra-enkripsi (plaintext JSON) tetap terbaca — dekripsi fallback ke parse plaintext bila bukan ciphertext valid, lihat `core/mcp_registry.py::_decrypt_env_json`.
  - **Pengecualian sadar #5 (disetujui owner, 2026-08-02):** `regex` untuk `tools/search.py::GrepTool` — regex dari argumen LLM dijalankan tanpa timeout ke semua file workspace, `requires_approval=False`; pattern catastrophic-backtracking (ReDoS, mis. `(a|a)+$`) bisa macetkan proses (audit produksi 2026-08-02). stdlib `re` tak punya timeout native, dan `asyncio.wait_for` tak bisa membatalkan regex CPU-bound yang sedang jalan (butuh mekanisme timeout di dalam engine regex itu sendiri). `regex` dipilih: drop-in replacement `re` dengan parameter `timeout=` native — diverifikasi langsung (bukan diasumsikan) bahwa pattern yang membuat `re.search()` hang >30s diinterupsi `TimeoutError` tepat di batas timeout via `regex` module, dan bahwa `regex` module SENDIRI (independen dari timeout) masih rentan ReDoS untuk sebagian pattern (`(a|a)+$`) — jadi timeout tetap wajib, bukan cuma ganti modul saja.
