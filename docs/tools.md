...
- Validasi `_validate_requirements` SEBELUM build (§ residual risk pip, lihat di bawah): tolak kosong, >20.000 byte, >200 baris, atau baris manapun (non-komentar) yang diawali `-` — memblokir opsi pip (`-e`, `--index-url`, `-r`, `-c`, dst) yang bisa mengalihkan sumber paket ke index tak tepercaya atau instalasi VCS/lokal arbitrer. **Audit 2026-09-25:** setiap baris kini juga harus lolos allowlist PEP 508 by-nama (`_requirement_line_ok`: nama, extras, penentu versi, marker, pin `--hash=sha256:…`) — URL langsung (`pkg @ https://…`, `https://…/x.whl`), VCS (`git+…`), path lokal dan `file://` sebelumnya LOLOS (tak diawali `-`) dan diinstal saat build ber-network, bertentangan dengan klaim kalimat ini.
