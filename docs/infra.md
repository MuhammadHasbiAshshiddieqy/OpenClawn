...
| `oidc_open_signup` | `False` | Keputusan 2026-09-26 — tanpa allowlist OIDC, pendaftaran akun BARU ditutup (user lama tetap bisa login, user pertama tetap bootstrap admin). `True` = perilaku lama (akun apa pun di IdP jadi member). Env `OPENCLAWN_OIDC_OPEN_SIGNUP=true` |
...
| `sandbox_runtime` | `"runc"` | **[§ IMPROVEMENT-Sandbox-Isolation-Parallelization.md Fase 5]** Runtime container untuk `code_run`/`shell_run` (`tools/sandbox.py`). Default `"runc"` (bawaan Docker) — TIDAK diteruskan sebagai `--runtime` sama sekali, perilaku lama tak berubah. Isi `"runsc"` (gVisor) via `OPENCLAWN_SANDBOX_RUNTIME` untuk isolasi lebih kuat; operator bertanggung jawab memastikan runtime itu terpasang di Docker daemon-nya. Firecracker/microVM sengaja tak didukung — butuh stack orkestrasi terpisah, di luar skop satu config knob |
| `sandbox_tmp_dir` | `None` | Keputusan 2026-09-26 — direktori skrip sementara `run_python`. Wajib volume bersama (path SAMA di app & daemon) saat daemon Docker di sidecar DinD (`docker-compose.sandbox.yml` → `/sandbox/tmp`). Env `OPENCLAWN_SANDBOX_TMPDIR` |
| `sandbox_persist_idle_ttl_sec` | `600` | **[§ IMPROVEMENT-Sandbox-Isolation-Parallelization.md Fase 3]** Sandbox persisten idle lebih lama dari ini → `SandboxReaper` men-`docker pause` (hemat CPU host, state tetap ada) |
| `sandbox_persist_destroy_ttl_sec` | `3600` | **[Fase 3]** Sandbox persisten tak dipakai lebih lama dari ini (terlepas state pause/running) → `SandboxReaper` men-destroy PERMANEN (`docker rm` + `docker volume rm`) |
| `sandbox_persist_max_containers` | `5` | **[Fase 3]** Batas sandbox persisten aktif SEKALIGUS — dicek `sandbox_persist_enable` sebelum membuat container baru. Menutup permukaan DoS baru yang tak ada di model ephemeral lama |
| `sandbox_reaper_tick_sec` | `60` | **[Fase 3]** Interval `SandboxReaper` mengevaluasi ulang semua sandbox persisten aktif |
