...
| **qa** | `false` | Review, test case, edge case | baca + tulis (`file_write`/`doc_write`/`pdf_write`), shell_run/code_run/**build_sandbox_image**/**sandbox_persist_enable** (sandboxed), `mcp__connector__*` |
| **dev** | `false` | Implementasi, fix, refactor | set penuh: baca/tulis/edit/patch (`file_write`/`doc_write`/`pdf_write`), shell_run/code_run/**build_sandbox_image**/**sandbox_persist_enable**, http_request, `mcp__connector__*` |
| **data** | `false` | Analisis, eksplorasi, statistik, insight, modeling dasar | baca + db_query (SELECT) + code_run/**build_sandbox_image**/**sandbox_persist_enable** (hitung statistik/modeling di sandbox) + `mcp__connector__*`; **tanpa tulis file** |
