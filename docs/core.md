...
- **Trust mode (§ user request otonomi):** `bypass_approval` dihitung SEBELUM eksekusi (`trust_mode` aktif, bukan autopilot, tool bukan `_TRUST_MODE_EXEMPT`, DAN bukan `policy_forces_approval` — lihat poin Policy Engine di bawah) — menentukan status yang di-emit ke UI: `AgentEvent(type="status", text="tool_trusted", ...)` (chip biasa + badge "trusted") alih-alih `text="approval"` (kartu Approve/Reject) untuk tool yang `requires_approval_effective` (`tool.requires_approval` ATAU `policy_forces_approval`). Diteruskan ke `_execute_tool` sebagai parameter, bukan dihitung ulang di sana.
- **Policy Engine (TODO.md § Prioritas 3):** `self.policy_engine.evaluate(tool_name, tool_input)` dipanggil DI SINI juga (bukan hanya `_execute_tool`) agar status UI konsisten dengan apa yang benar-benar terjadi saat eksekusi. `policy_forces_approval` (`action == "require_approval"`) membuat `requires_approval_effective = True` walau `tool.requires_approval` statis `False`, DAN memaksa `bypass_approval = False` — trust mode TIDAK BISA melewati approval yang dipaksa policy (§ `docs/security.md` § `PolicyEngine` untuk alasan desain).
...
Eksekusi satu tool dengan jaring pengaman §1.3: cek keberadaan → cek izin role → **validasi input vs schema** (`_validate_tool_input`) → **Policy Engine** (`PolicyEngine.evaluate`, TODO.md § Prioritas 3) → approval jika perlu → jalankan dalam `asyncio.wait_for(timeout=tool_timeout_sec)` dengan try/except yang mengubah exception/timeout apa pun menjadi `{"error": ...}` anggun → potong output seragam (`_truncate_tool_output`) → catat telemetri (`ToolAudit.record`). Satu tool yang gagal/menggantung tidak menjatuhkan turn.

Policy Engine dicek SEBELUM approval: `action="deny"` → tolak SEBELUM approval sempat dipanggil sama sekali (lebih ketat dari sekadar meminta approval untuk sesuatu yang harus ditolak mutlak). `action="require_approval"` → `policy_forces_approval=True`, memaksa masuk cabang approval di bawah meski `tool.requires_approval` statis `False` (mis. `shell_run`).
